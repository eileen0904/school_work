...
public:
	...
	void BFS(WeightedGraphVertex<V, E> *v)
	{
        ...
        {
            std::cout<<cur<<" ";
            ListNode<WeightedGraphEdge<V, E> *> *e = (*(cur->getData()))[0];
            while(e != NULL)
            {
                if(l->exist(e->getData()->getAnotherEnd(cur->getData())) == NULL)
                    l->addFromTail(e->getData()->getAnotherEnd(cur->getData()));
                e = e->getNext();
            }
            cur = cur->getNext();
        }
	...
	void DFS(WeightedGraphVertex<V, E> *v)
	{
	    ...
	    while(cur != NULL)
        {
            ...
            {
                if(l->exist(e->getData()->getAnotherEnd(cur->getData())) == NULL)
                    l->addAfter(cur, e->getData()->getAnotherEnd(cur->getData()));
                e = e->getNext();
            }
            cur = cur->getNext();
        }
...
int main()
{
	WeightedGraph<int, int> *g = new WeightedGraph<int, int>();
	...
	g->addLink((*g)[3], (*g)[9], 8);
	g->addLink((*g)[4], (*g)[5], 9);
	g->addLink((*g)[9], (*g)[10], 10);
	g->addLink((*g)[9], (*g)[11], 11);
	//g->adjMatrix();
	g->adjList();
	g->BFS((*g)[0]);
	std::cout<<std::endl;
