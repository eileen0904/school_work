    ...
    AGAIN:
        ...
        for (int i = 0; i < epoll_events_count; i++){
            ...
            if(sockfd == listenfd){
                ...
                char *usr_name = login(c, newfd);
                strcpy(clients[ID].name, usr_name);
                ...
                if (SC == 0){
                    ...
                    shell(newfd, ser_fifo_fd[ID], cli_fifo_fd[ID], ID, usr_name, c);
                    exit(0);
                }
                ...
                for (int j = 0; j < MAX_CLIENTS; j++) {
                    if (sockfd == cli_fifo_fd[j]) {
                        ...
                        if (len < 0) {
                            ...
                            else if (strncmp(buff, "gyell", 5) == 0){
                                redisReply *reply = NULL;
                                char *msg, usr_name[25] = {}, group_name[MAX] = {};
                                strcpy(usr_name, clients[j].name);

                                msg = gyell(buff, group_name, usr_name, c);
                                if (strcmp(msg, "Group not found!\n") == 0)
                                    write(ser_fifo_fd[j], "Group not found!\n", strlen("Group not found!\n"));
                                else{
                                    reply = redisCommand(c, "LRANGE %s_group 0 -1", group_name);
                                    char online_usr[MAX_CLIENTS][25] = {};
                                    if (reply && reply->type == REDIS_REPLY_ARRAY){
                                        for (int k = 0; k < reply->elements; k++){
                                            for (int m = 0; m < MAX_CLIENTS; m++){
                                                if (ser_fifo_fd[m] != -1 && strcmp(clients[m].name, reply->element[k]->str) == 0){
                                                    write(ser_fifo_fd[m], msg, strlen(msg));
                                                    break;
                                                }
                                            }
                                        }
